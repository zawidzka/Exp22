...
convertCSVtoFCS(CSVfiles = CSVfiles[-6], csvDirectory = CSVdirPath, csv2fcsDirectory = FCSdirPath, fileName = fileName[-6])
...
FCSfiles <- FCSfiles[!grepl('PB', FCSfiles)]
FCSfiles
...
sample_md <- sample_md[!grepl('PB', sample_id)]
sample_md
...
saveRDS(sce, file = "SCE_BM_LN_TUM.rds")
